...
| `--inner-loop` | False | 启用内环闭环跟踪（平滑模式下小循环迭代速度控制直到误差收敛） |
...
2. **时间计算**：根据时间戳或固定频率计算每步之间的时间间隔（优先使用数据中记录的 `collection_frequency`）
...
- **时间同步**：优先使用时间戳保持原始时序，否则使用固定频率（优先使用 `collection_frequency`；旧数据缺失时回退到默认值）
- **速度控制**：通过调整时间间隔实现回放速度控制
- **内环闭环跟踪（可选）**：平滑模式下每步进入小循环，持续发送速度命令，直到位置/姿态误差收敛再进入下一步
...
4. **轨迹完整性**
   - 如果轨迹数据不完整或时间戳异常，程序会回退到固定频率（优先使用 `collection_frequency`；旧数据缺失时回退到默认值）
   - 部分轨迹回放时，确保起始和结束步数有效

5. **回放速度**
6. **关节误差打印**
   - 回放过程中会实时打印目标关节角与实际关节角的误差（单位：rad）
   - 误差用于诊断跟踪精度，不影响控制逻辑
...
| **回放不准确** | 时间戳缺失或数据不完整 | 程序会自动使用固定频率（优先使用 `collection_frequency`；旧数据缺失时回退到默认值），这是正常的 |
