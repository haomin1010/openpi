...
def init_train_state(
    ...
    if config.cls_train and config.frozen:
        cls_exclusive_freeze = nnx.All(
            nnx.Param,
            nnx.Not(nnx_utils.PathRegex(r".*(cls_proj|cls_param|action_cls_head).*")),
        )
    ...
    def init(rng: at.KeyArrayLike, partial_params: at.Params | None = None) -> training_utils.TrainState:
        ...
        if config.cls_train and config.frozen:
            effective_trainable_filter = nnx.All(
                nnx.Param,
                nnx_utils.PathRegex(r".*(cls_proj|cls_param|action_cls_head).*"),
            )
        else:
...
def train_step(
    ...
    def loss_fn(
        ...
        chunked_loss = model.compute_loss(rng, observation, actions, train=True, cls_train=config.cls_train, gamma=config.vic_gamma, t_step=state.step)
